    ...
    (async () => {
      ...
      //  console.log(ipfsCIDs[i][1]);
          if(ipfsCIDs[i][1].length < 50) {
            ...
            var test = String(res);
            temp.push({ poem : res[0], prompt : String(res[1]) });
          }
        }
      }
      setPoemList(temp);
  ...
  <div class="container px-5 py-12 mx-auto">
    <div class="flex flex-wrap -m-4">
      {poemList.map(poem => (
        ...
        <div class="h-full bg-white bg-opacity-75 px-8 pt-16 pb-12 rounded-lg overflow-hidden relative border-2 border-[#111827] shadow-custom">
          <h1 class="title-font sm:text-2xl text-3xl font-sor font-medium text-gray-900 mb-3 text-center">{poem.prompt}</h1>
          <NewlineText class="leading-relaxed text-xl mb-3 font-est" text={poem.poem}></NewlineText>
          </div>
        </div>
      ))}
    </div>
